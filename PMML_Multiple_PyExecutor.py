...
class PMMLManager:
    ...
    def is_single_model(self) -> bool:
        return len(self.models_mapping) == 1

    @property
    def single_segment_name(self) -> str:
        return list(self.models_mapping.keys())[0]

    def load_evaluator(self, segment: str):
        """Carga en memoria Java UNICAMENTE el modelo del segmento que se va a procesar."""
        pmml_file = self.models_mapping.get(segment)
        if not pmml_file:
            raise KeyError(f"El segmento '{segment}' carece de definición de modelo en el JSON.")
            
        full_path = os.path.join(self.general_path, pmml_file)
        logger.info(f"Cargando en JVM el modelo PMML para segmento '{segment}'...")
        return make_evaluator(full_path)

    def get_pmml_schema(self, evaluator) -> Dict[str, str]:
        schema = {}
        for field in evaluator.getInputFields():
    ...
    def get_input_order(self, evaluator) -> List[str]:
        return [str(field.getName()) if hasattr(field, 'getName') else str(field.name) for field in evaluator.getInputFields()]

    def score_segment(self, evaluator, df_segment: pd.DataFrame, segment: str, chunk_size: int = 1000) -> pd.DataFrame:
        input_order = self.get_input_order(evaluator)
        ...
        logger.info(f"Ejecutando scoring JPMML para segmento '{segment}' ({total_rows} registros)...")
        
        if total_rows > chunk_size:
            logger.info(f"Segmento '{segment}' supera {chunk_size} registros. Evaluando en micro-lotes...")
            chunk_results = []
            for start_idx in range(0, total_rows, chunk_size):
                sub_segment = df_segment.iloc[start_idx : start_idx + chunk_size].copy()
                sub_input = sub_segment[input_order]
                
                df_predictions = evaluator.evaluateAll(sub_input)
            ...
            df_predictions = evaluator.evaluateAll(df_scoring_input)
...
def PMML_PyExecutor(
    ...
    if pmml_manager.is_single_model:
        ...
        try:
            evaluator = pmml_manager.load_evaluator(segment)
            if df_types is not None:
                df_transformed = data_transformer(df_raw, csv_schema_mapping)
            else:
                pmml_native_schema = pmml_manager.get_pmml_schema(evaluator)
                df_transformed = data_transformer(df_raw, pmml_native_schema)

            del df_raw
            gc.collect()

            df_scored = pmml_manager.score_segment(evaluator, df_transformed, segment, chunk_size=chunk_size)
            df_scored["executed_segment"] = segment
            processed_segments_chunks.append(df_scored)

            # Destruir instancia en Java
            del evaluator
            gc.collect()
        except Exception as e:
        ...
        for segment in unique_segments:
            ...
            try:
                # 1. Cargar ÚNICAMENTE el modelo del segmento actual
                evaluator = pmml_manager.load_evaluator(segment)
                ...
                # 2. Realizar scoring por bloques
                df_chunk_scored = pmml_manager.score_segment(evaluator, df_chunk_transformed, segment, chunk_size=chunk_size)
                df_chunk_scored["executed_segment"] = segment
                processed_segments_chunks.append(df_chunk_scored)

                # 3. UNLOAD EXPLÍCITO: Eliminar el evaluador de la JVM inmediatamente
                del evaluator
                gc.collect()
                logger.info(f"Modelo PMML del segmento '{segment}' liberado exitosamente de la memoria Java.")
